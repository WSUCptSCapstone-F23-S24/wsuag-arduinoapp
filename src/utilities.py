...
def make_constant_csv(in_path):
    ...
    for file in r_in_data:
        ...
        for s, c in zip(sp, color):
            ...
            # Make list of extracted data for one color
            data = [date, time, date_time, spectrum, mean, median, std, max,
                    p95, p90, p85, rep_pic]
    ...
    header = ['date', 'time', 'date_time', 'sprectrum', 'mean', 'median',
              'std', 'max', 'p95', 'p90', 'p85', 'rep_pic']
...
def slice_into_boxes(image_path,
                     out_folder,
                     x,
                     y,
                     length,
    ...
    cv2.rectangle(crop_img,
                  (x, y),
                  (int(x+length),
                   int(y+(length*(height_base/length_base)))),
                  (0, 0, 256),
                  1)
    ...
    if render_rectangle:
        ...
        crop_box_1_debug = crop_img_debug[
                        (box_1_y_1 - zoom_out):(box_1_y_2 + zoom_out),
                        (box_1_x_1 - zoom_out):(box_1_x_2 + zoom_out)]
    ...
    out_path = (out_folder +
                ...
                "_box_1" + os.path.splitext(image_path)[1])
    print("out path:")
    print(out_path)

    cv2.imwrite(out_path, crop_box_1)
    if render_rectangle:
        out_path_debug = (out_folder +
                          "/debug/" +
