...
df = pd.read_csv("./test_results/curve.csv",parse_dates=['date'], index_col='date',dayfirst=True)

# print(df["date"])
df.plot(kind = "bar", x = "index.date", y="mean", use_index=True,)
plt.show()
# plt.plot(df.index, df["p95"], 'graphs') # Still using 'bo' for blue circles
# plt.show()
...
# ax.set_xticks(range(len(df.index.date)))  # Set x-ticks to match the number of points
